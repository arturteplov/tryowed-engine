...
// ─── Build personalized scan steps ───────────────────────────────────────────

function buildScanSteps(states: string[]) {
  const steps: { label: string; duration: number }[] = [
    { label: "Searching federal tax credit databases", duration: 1500 },
  ];
  const statesToSearch = states.length > 0 ? states : ["your state"];
  for (const s of statesToSearch) {
    steps.push({ label: `Checking unclaimed property in ${s}`, duration: 1400 });
  }
  steps.push(
    { label: "Scanning active class action settlements", duration: 1600 },
    { label: "Reviewing pension and savings bond databases", duration: 1400 },
    { label: "Analyzing your eligibility", duration: 1200 },
  );
  return steps;
}
...
export default function ResultsPage({
  ...
  const scanTotalMs = buildScanSteps(states).reduce((s, st) => s + st.duration, 0) + 600;

  useEffect(() => {
    const timer = setTimeout(() => setPhase("results"), scanTotalMs);
